...
const Persontype = () => {
  ...
  const [Persontypemodalpopup, setPersontypemodalpopup] = useState(false);
  const [disabledbtn, setDisabledBtn] = useState(false);
  const[persontypedatapopup,setPersontypedata]=useState({
    columns,
    data
  });
  ...
  const editdata=(row, e) =>{
    ...
    setPersontypemodalpopup(!Persontypemodalpopup);
  }
 ...
 const onUpdateItem = i => {
  ...
  const newdata = persontypedatapopup["data"].map((item) => {
    if (item.id === i) {
      const updatedItem = {
        ...item,
  ...
  setPersontypedata({ ...persontypedatapopup, ["data"]: newdata });
  resetvalues();
  setPersontypemodalpopup(!Persontypemodalpopup);
 ...
 const savedata = () => {
  ...
  if(state.description.length==0){
    ...
    dataservice("persontype",{ requestfor: 'persontype' ,
    data:{flag:state.id==""?"i":'u',description:state.description,price:state.price,
      ...
      if(data.resultkey==1){
        if(state.id!=""){
          ...
          onUpdateItem(state.id);
            ...
            setPersontypedata({ ...persontypedatapopup, ["data"]: [...persontypedatapopup.data,newuser ] });
            resetvalues();
            notify(1,`Data save successfully.`);
        }
...
const getpersontypedata=()=>{
  dataservice("persontype",{ requestfor: 'getpersontypewisepricing' ,data:{flag:'a'}})
  .then(function(data) {
    if(data.resultkey==1){
      var result=data.resultvalue;
       ;
      setPersontypedata({ ...persontypedatapopup, ["data"]: result });
    }else{
   ...
   <CRow>
      <CCol xl={12}>
        <CCard>
          ...
          <CCardBody>
          <CModal show={Persontypemodalpopup} onClose={() => setPersontypemodalpopup(!Persontypemodalpopup)} size="lg">
              ...
              <CModalFooter>
                ...
                <CButton color="secondary" onClick={() => setPersontypemodalpopup(!Persontypemodalpopup)}>Cancel</CButton>
              </CModalFooter>
            </CModal>
